<?php
 require '../header.php';
 if(! $_SESSION['isAdmin']){
  header("Location: http://dev.bmedia.com");
 }
?>
  <h3>Master Page</h3>
...
<div class="row">
    ...
    <div id="menu3" class="tab-pane fade">
      ...
      <button type="button" class="btn btn-sm btn-primary float-right" data-toggle="modal" data-target="#ad_modal"> <i class="fa fa-plus"></i> Add Advertise</button>
      
      </p>
      <br>

      <table class="table table-striped table-bordered" id="ad_table">
    <thead>
      <tr>
        <th>#</th>
        <th>Title</th>
...
<script>
  var fields=["song_name","feature_artist","label","artist_id","song_added_date","status"];
  var videofields=["video_name","linked_song_id","youtube_embed_link","content_access","added_on","status"];
  ...
  if(videopage_active == 1){
   ...
   $(".tab-content div").eq(1).addClass('in active');
...
function getAds(){
  $.ajax({
     ...
     $("#ad_table tbody").append('<tr><td>'+(j+1)+'</td><td>'+ads[j]['title']+'</td><td>'+ads[j]['subtitle']+'</td><td class="text-center">'+ads[j]['position']+'</td><td>'+ad_span+'</td><td>'+ads[j]['url']+'</td>'+imagetd+'<td class="ad_table_action"><a class="edit ad_edit" href="" data-id='+j+' title="Edit" data-toggle="tooltip"><i class="material-icons">&#xE254;</i></a><a class="delete editor_remove" data-id='+ads[j]['id']+' href="" title="Delete" data-toggle="tooltip"><i class="material-icons">&#xE872;</i></a></td></tr>'); 
    }
         }
...
getAds();
 ...
 // edit add 
 $('#ad_table').on('click', 'a.ad_edit', function (e) { 
   //adList
 let id=$(this).data('id');
...
.then((willDelete) => {
  ...
  //  $(this).closest("tr").remove();
    cells[5].innerHTML ='<span class="badge badge-danger">removed</span>';    
