...
<div class="table-responsive">
    ...
    <div id="menu2" class="tab-pane fade">
      ...
      <tr>
        <td colspan="2"><span class="glyphicon glyphicon-question-sign"></span>Help</td>
...
<div id="menu4" class="tab-pane fade">
    ...
    <div id="menu3" class="tab-pane fade">
      <h3>Advertisement</h3>
      <p><button type="button" class="btn btn-sm btn-primary" data-toggle="modal" data-target="#ad_modal"> <i class="fa fa-plus"></i> Add Advertise</button>
      </p>
      <br>
        <div class="table-responsive">
              <table class="table table-striped table-bordered" id="adver_table">
            <thead>
              <tr>
                <th>#</th>
                <th>Title</th>
...
<!-- Artist Modal start -->
<div class="modal fade" id="artist_modal" role="dialog">
    <div class="modal-dialog modal-lg modal-dialog-center">
    <!-- Modal content-->
      <div class="modal-content">
        <div class="modal-header">
        ...
        <div class="modal-body">
        <form novalidate="true" id="artistupdate">
            <div class="row">
            <div class="col-md-6">
                <label for="usr">Artist Name:</label>
            ...
            <div class="modal-footer">
              <button type="button" class="btn btn-success" id="update_artist" data-id=""  onclick="updateRowArtist()" data-dismiss="modal">Update</button>
              <button type="button" class="btn btn-primary" data-dismiss="modal">Close</button>
            </div>
          </div>
      </form>
...
<div class="modal fade" id="album_modal" role="dialog">
    ...
    <!-- Modal content-->
      <div class="modal-content">
        ...
        <div class="modal-body">
        <form novalidate="true" id="albumupdate">
        <div class="row">
        <div class="col-md-6">
        <label for="usr">Album Name:</label>
          ...
          <input type="hidden" class="form-control" id="album_id" name="album_id">
            ...
            <div class="col-md-6">
            <label for="usr">Artis:</label>
        <input type="text" class="form-control" id="artist_eid" name="artist_eid">
        </div>
          </div>
        <br>
          <div class="row">
          ...
          <input type="text" class="form-control" id="album_price"  name="album_price">
          </div>
            
          
            <div class="col-md-6">
                <label for="usr">Status:</label>
                <select class="form-control" id="album_status" name="album_status">
                    <option value='' disabled>select</option>
                    <option value='live'>Live</option>
                    <option value='pending'>Pending</option>
                    <option value='removed'>Removed</option>
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-success" id="update_album" data-id=""  onclick="updateRowAlbum()" data-dismiss="modal">Update</button>
          <button type="button" class="btn btn-primary" data-dismiss="modal">Close</button>
        </div>
      </div>
      </form>
...
<script>
  ...
  var artistfields  =   ["artist_id","artist_name"];
  var albumfields   =   ["album_id","album_name","artist_eid","artist_ename","album_price","album_status"];
  ...
  function updateAjaxCall(postdata){
   ...
   $.ajax({
    ...
    // artist edit
    function updateRowArtist(){
    
        var updaterow   =  $("#update_artist").data("id"); // will return the number 123
        var a           =   $("#art_table tbody tr");
        var currenttable_row    =   a[updaterow];
       // console.log(currenttable_row);
        var cells   =   currenttable_row.getElementsByTagName("td");
        var datas   =   $("#artistupdate").serialize();
        updateAjaxCall(datas);
        
        for(let i=0; i<2; i++){
            
                var updatevalue     =   $("#"+artistfields[i]).val();
                //alert(updatevalue);
            ...
            swal("Values Updated Successfully", {icon: "success",closeModal: true, });
    }
    
    //album edit
    function updateRowAlbum(){
    
        var updaterow   =   $("#update_album").data("id"); // will return the number 123
        var a           =   $("#albm_table tbody tr");
        var currenttable_row    =   a[updaterow];
       // console.log(currenttable_row);
        var cells   =   currenttable_row.getElementsByTagName("td");
        var datas   =   $("#albumupdate").serialize();
        updateAjaxCall(datas);
        
        for(let i=0; i<6; i++){
            
            var updatevalue     =   $("#"+albumfields[i]).val();
            //alert(updatevalue);
            ...
            if(i==5){
                 cells[i].innerHTML  = '<span class="badge badge-'+colorList[updatevalue]+'">'+updatevalue  +'</span>';  
            } 
  ...
  $.ajax({
       ...
       for(let j=0;j<ads.length;j++){
         ...
         let ad_span='<span class="badge badge-'+adListText[ad_status]['color']+'">'+adListText[ad_status]['text']+'</span>';
                     ...
                     $("#adver_table tbody").append('<tr><td>'+(j+1)+'</td><td>'+ads[j]['title']+'</td><td>'+ads[j]['subtitle']+'</td><td class="text-center">'+ads[j]['position']+'</td><td>'+ad_span+'</td><td>'+ads[j]['url']+'</td>'+imagetd+'<td class="ad_table_action"><a class="edit ad_edit" href="" data-id='+j+' title="Edit" data-toggle="tooltip"><i class="material-icons">&#xE254;</i></a><a class="delete editor_remove" data-id='+ads[j]['id']+' href="" title="Delete" data-toggle="tooltip"><i class="material-icons">&#xE872;</i></a></td></tr>'); 
            }       
         }
...
getAds();
 ...
 // edit add 
 $('#adver_table').on('click', 'a.ad_edit', function (e) { 
   //adList
 ...
 * --------------------------------------------*/ 
    // Edit record video
    $('#art_table').on('click', 'a.editor_edit', function (e) {
       var row      =   $(this).closest('tr');
       
       let videoid  =   $(this).data('id');
       $("#artist_id").val(videoid);
       
        var data        =   row[0]['cells'];
        var rowindex    =   row[0]['rowIndex']-1;
        
        $('#update_artist').attr('data-id', rowindex);

        $.each(data, function( index, value ) {
            if(index < 8){
                let getid       =   artistfields[index];
                let fieldvalue  =   value['innerText'];
                console.log(getid,fieldvalue);
                $("#artist_modal #"+getid).val(fieldvalue);
            }
        });
    ...
    // Edit record video
    $('#albm_table').on('click', 'a.editor_edit', function (e) {
           var row=$(this).closest('tr');
           let videoid=$(this).data('id');
           $("#video_id").val(videoid);
            var data=row[0]['cells'];
            var rowindex=row[0]['rowIndex']-1;
            $('#update_album').attr('data-id', rowindex);
    
            $.each(data, function( index, value ) {
              if(index < 8){
                let getid=albumfields[index];
                let fieldvalue=value['innerText'];
                //console.log(getid,fieldvalue);
                $("#album_modal #"+getid).val(fieldvalue);  
...
var text=arr[data.content_access] ? arr[data.content_access] : "--";
            ...
            { data: null, render: function ( data, type, row ) {
                let arr={"live":"success","pending":"warning","removed":"danger"};
                return '<a class="edit editor_edit" href="" data-id='+data.file_id+' title="Edit" data-toggle="tooltip"><i class="material-icons">&#xE254;</i></a><a class="delete editor_remove" data-id='+data.file_id+' href="" title="Delete" data-toggle="tooltip"><i class="material-icons">&#xE872;</i></a>';
            } }
  ...
  $('#example_newsletter').DataTable( {
    ...
    // Delete a artist record
    $('#art_table').on('click', 'a.editor_remove', function (e) {
    ...
    e.preventDefault();
     ...
     // Delete a album record
     
    $('#albm_table').on('click', 'a.editor_remove', function (e) {
